...
public class LoadManager4 : MonoBehaviour
{
    ...
    {
#if UNITY_EDITOR
        Debug.Log("<color=white>LoadManager4</color> - Awake. Getting all needed in SaveManager");
#endif
        // Character
        ...
        {
            Vector3 characterPosition = PlayerPrefsExtra.GetVector3("S4 Character Position", spawnPoint.position);
        ...
        else
        {
            ...
            SaveManager.ResetKeyCount(); // Reset Key Count on continue or not because we dont need keys here in stage 4
            ...
            if (SaveManager.syrupD == 1 || SaveManager.syrupD == 2) DisableItems("item", 1);
            if (SaveManager.riddleD == 1 || SaveManager.riddleD == 2) DisableItems("item", 2);
...
#if UNITY_EDITOR
        Debug.Log("<color=white>LoadManager4</color> - Prefab: " + prefab.name + " - SpawnPoint: " + SaveManager.currentStage);
        Debug.Log("<color=white>LoadManager4</color> - Game Difficulty: " + SaveManager.gameDifficulty);
        Debug.Log("<color=white>LoadManager4</color> - Framerate: " + SaveManager.framerateOn);
        Debug.Log("<color=white>LoadManager4</color> - Camera Distance: " + SaveManager.cameraDistance);
        Debug.Log("<color=white>LoadManager4</color> - Current Immunity: " + SaveManager.currentImmunity);
        Debug.Log("<color=white>LoadManager4</color> - Look Sensitivity: " + SaveManager.lookSensitivity);
#endif
    }
    ...
    void OnPlayableDirectorStopped(PlayableDirector aDirector)
    {
        ...
        {
            GameObject.FindGameObjectWithTag("PlayerHide").tag = "Player";
